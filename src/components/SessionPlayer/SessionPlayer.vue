<template>
    <div class="SessionPlayer">
        <div class="main">
            ...
            <div class="next">
                <button class="btn btn-outline-primary btn-next btn-sm"
                        @click="onClickBtnFinishSession">
                    Завершить сессию
                </button>
            </div>
            <div class="next">
                <button class="btn btn-outline-primary btn-next btn-sm"
                        @click="onClickBtnPauseSession">
                    Пауза
                </button>
            </div>
            <div class="next">
            ...
            {{!!this.session ? this.session.stack : ''}}
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        onClickFinishCycle() {
            if (this.session.stack > 0) this.session.stack.shift();
        },
        saveHistoryItem() {
                ...
                switch (typeChild) {
                    ...
                    case 'loopList': {
                        ...
                        if (childrenOfChildAmount > 0) {
                            let maxCount = this.mapKeyNodes[curr.key].list[curr.counter % childrenAmount].attrs.loopCount.value * childrenOfChildAmount;
